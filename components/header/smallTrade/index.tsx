...
export default function SmallTrade() {
  ...
  useEffect(()=>{
    if (roomId !== null && roomId !== '') {
      ...
      const tokenKey = Object.keys(localStorage).find(key => key.startsWith(roomId));
      if (tokenKey) {
        ...
        if (tokenKey.split('-')[1] === 'streamer') setStreamUrl(`/stream/${roomId}/host`);
            ...
            <div className='flex h-full w-full flex-col rounded-2xl shadow-sky-50 dark:bg-darkPrimary'>
              <div className='bg-h flex justify-between px-2 pt-2'>
                {/* <div className='flex w-full justify-center'>
                  {!isMobile && <p>{roomId}</p>}
                  <CopyClipboard
                    text={`https://www.paxintrade.com/stream/${roomId}`}
                  >
              ...
              {
                isHost ? <SmallHost token={token} roomId={roomId} /> : <SmallWatch roomId={roomId} token={token} />
              }
